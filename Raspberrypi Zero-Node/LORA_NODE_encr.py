...
def get_node_parameter(correctedPPM):
   ...
   #print(value_pin)  
    ...
    # msg = '"i":{},"t":{},"h":{},"l":{},"p":{}'.format(1,t,h,ldr.value,round(correctedPPM))
    if correctedPPM > 400:
       buzzer.beep()
    else: 
       buzzer.off()
...
class mylora(LoRa):
    ...
    def on_rx_done(self):
        ...
        if decoded=="INF             ":
            ...
            result =  '== SEND: DATA FROM NODE {} HUM: {} TEM: {}  LIGHT: {} PPM:  {}   |  Encoded: {}'.format(1,h,t,ldr.value,round(correctedPPM), encoded.decode("utf-8",'ignore'))
            print(result)
        if decoded=="ACK             ":
            print("\n")
