...
class mylora(LoRa):
    ...
    def on_rx_done(self):
        ...
        BOARD.led_off()
        if decoded=="INF             ":
            logging.info("Received data request INF - going to send mens data from node 1: ")
            time.sleep(4)
            ...
            logging.info("== SEND: DATA FROM NODE {} HUM: {} TEM: {}  LIGHT: {} PPM:  {}   |  Ended: {}".format(1,h,t,ldr.value,round(correctedPPM), encoded.decode("utf-8",'ignore')))  
        if decoded=="ACK             ":
            print("\n")
...
lora.set_bw(int(config['Node1']['band_width']))
...
logging.info("Node 1 start: {}".format(lora))
